...
public class AstralDisplayBlockEntityRenderer<T extends AstralDisplayBlockEntity> implements BlockEntityRenderer<T> {
	...
	public void render(T entity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
		if (entity.getWorld() != null) {
			float time = ((float) (MinecraftClient.getInstance().world.getTime() % 2400000L) + MinecraftClient.getInstance().getTickDelta())*5f;

			matrixStack.push();
			matrixStack.translate(.5f, .5f,.5f);


			matrixStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(time));
			...
			ParticleBuilders.create(LodestoneParticles.WISP_PARTICLE)
				.setScale(SIZE * (1 + random.nextFloat()))
				.setAlpha(0, 0.2f, 0)
